...
if st.button("Predict"):
    ...
    # Make 
    data_transformed = preprocessor.transform(data)  # Transform data before prediction
predicted_demand = demand_model.predict(data_transformed)[0]

    predicted_demand = demand_model.predict(data)[0]
    predicted_discount = discount_model.predict(data)[0]

    # Display results
    st.success(f"📊 Predicted Demand: {predicted_demand}")
    st.success(f"💰 Predicted Discount: {predicted_discount}%")
